...
| **M4-3** | 二维码 | ✅ | 见下 | 前端 `qrcode` 画 canvas（无后端接口）：用 **jsQR 真扫**，页面 canvas 与下载的 1024×1024 PNG 都解码出 `http://localhost:8080/{code}`，与 `short_url` 逐字相等；配色深墨 + 暖奶油（≈19:1，非珊瑚）；无头 Chrome 里「下载二维码」按钮真的落盘 PNG |
