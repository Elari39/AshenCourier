...
> `d4d2957`（N9）→ run `35484563846`（`backend` 59s / `frontend` 36s / `smoke` 1m35s）。
...
## 16. 未完成清单（2026-09-19 盘点，2026-09-20 结清）

> 这一节回答「还剩什么」。基线是 `d76db54`（M0–M4 十二个批次全部完成并推送、CI 全绿）。
> 结论：**M0–M4 没有遗留** —— 剩下的全部是「按计划推迟的 M5」「计划里标注可选的」，
> 以及三条我在实施中发现的自动化缺口（计划里没写，属于我的判断）。
>
> **2026-09-20 更新：这一节已结清。** §16.2 的三项全部做完（N1 / N8 / N9），
> §16.1 里 M5-1 / M5-2 / M5-3 前半均已落地，M5-3 后半拍板为「不做」（§18.5），
> M5-4 维持「先别做」。§16.3 的三条自动化缺口也已全部补上（N1 / N4 / N3）。
> 也就是说：**计划里写下的每一条都已经有了结论** —— 要么做完，要么明确不做并写明理由。

### 16.1 M5 的开工前置条件（内容与验收见 §8）

| 批次 | 迁移 | 前置条件 | 状态（2026-09-20） |
| --- | --- | --- | --- |
...
| **M5-3 自定义域名** | `000006_domains` | 真上线需要域名与证书（certbot / ACME），但**本机验收不需要**：`curl -H 'Host: a.local'` 即可 | 代码侧前半 ✅ **N6-1 / `0456154`**；后半（短码唯一约束按域）**已拍板不做 —— §18.5** |
| **M5-4 多租户** | `000008_orgs`（**破坏性**，需回填 + 双写窗口） | 计划里唯一建议「先别做」（§8 结论），等真实多人协作需求 | ⏸ 不做（唯一保留的未做项，理由见 §8） |
...
| 项 | 结果 |
| --- | --- |
| M4-3 **方案 B**：后端 `GET /api/links/{code}/qr.svg` | ✅ **N8 / `adc845a`**（规格见 §19.1）。方案 A 仍在（详情页 canvas + 下载 PNG），两者并存：A 管「在页面上看」，B 管「被别处引用」 |
| **`/metrics` 零依赖文本端点** | ✅ **N9 / `d4d2957`**（规格见 §19.2）。§13 当初的两个前置条件（仅内网可达 + 进保留字表）逐条落实：nginx `= /metrics { return 404; }` + `reserved.go` |
| **`POSTGRES_TEST_DSN` 的 store 集成测试** | ✅ **N1 / `da6731d`**。CI 的 `backend` job 挂上 PG service，`internal/store/postgres` 真跑 **1.271s**（不设 DSN 时整体跳过，只有零点几秒 —— 时间本身就是「真的连上了」的证据） |
...
纯文档批次，把 N7–N9 的实测结果补齐到该补的地方（代码在这一批里**一行没改**）：

| 落点 | 内容 |
| --- | --- |
| §0 批次表 | 补 N7 / N8 / N9 三行（commit + 实测验收），并把三次 CI run 的结论并列在表下的引用块里 |
| §16 | 标题改成「已结清」；§16.1 的 M5-3 后半改指 §18.5（已拍板不做）；§16.2 的三项从「已立项」改成「已做完」 |
| §16.5 索引 | 批次计数与「验收记录 ①–⑳」的口径对齐 |
| `README.md` | 「最近的实测」指向 N9 的 run `35484563846`（并保留 N8 那次作为对照） |
